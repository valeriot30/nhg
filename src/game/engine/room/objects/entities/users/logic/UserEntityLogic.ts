...
export default class UserEntityLogic extends EntityLogic {
  ...
  public registerEvents() {
    ...
    let staticContainer = Engine.getInstance()
      .getUserInterfaceManager()
      .getUIComponentManager()
      .getComponent(UIComponent.StaticContainerUI) as StaticContainerUI;
    UserEntityVisualization.Avatar?.Container.on("user-position-changed", () =>
      this.onPositionChanged()
    );
    UserEntityVisualization.Avatar?.Container.on("user-started-typing", () =>
      this.userToggleTyping(true)
    );
    UserEntityVisualization.Avatar?.Container.on("user-stop-typing", () =>
      this.userToggleTyping(false)
    );
  }
  ...
  public setAvatarContainer() {
    let UserEntityVisualization = this.entity.getVisualization() as UserEntityVisualization;
    let avatarContainerUI = Engine.getInstance()
      .getUserInterfaceManager()
      .getUIComponentManager()
      .getComponent(UIComponent.AvatarContainerUI) as AvatarContainerUI;

    let dimension = new Point(
      UserEntityVisualization.Avatar?.Container.height!,
      UserEntityVisualization.Avatar?.Container.width!
    );

    let position = new Point(
      UiUtils.getGlobalPosition(UserEntityVisualization.Avatar!.Container).tx +
        dimension.getY() +
        AvatarData.AVATAR_LEFT_TYPING_OFFSET,
      UiUtils.getGlobalPosition(UserEntityVisualization.Avatar!.Container).ty -
        dimension.getX() +
        AvatarData.AVATAR_TOP_TYPING_OFFSET
    );
